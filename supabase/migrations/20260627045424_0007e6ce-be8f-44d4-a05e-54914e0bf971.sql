...
CREATE UNIQUE INDEX IF NOT EXISTS returns_workspace_return_number_key ON public.returns (workspace_id, return_number);
CREATE INDEX IF NOT EXISTS returns_workspace_tracking_idx ON public.returns (workspace_id, tracking_number);
CREATE INDEX IF NOT EXISTS returns_workspace_order_number_idx ON public.returns (workspace_id, order_number);
...
CREATE POLICY "Members view connections" ON public.connector_connections FOR SELECT TO authenticated USING (workspace_id = private.current_workspace_id());
...
CREATE POLICY "Owners update connections" ON public.connector_connections FOR UPDATE TO authenticated USING (workspace_id = private.current_workspace_id() AND private.has_role(auth.uid(), 'Owner'::public.app_role)) WITH CHECK (workspace_id = private.current_workspace_id());
